...
#' @param min  integer,Default is False and min is calculated based on mean,demand and leadtime unless set manually
...
sim_min_Q<- function (demand, mean=FALSE, sd=FALSE, leadtime, service_level, initial_inventory_level = FALSE, min= FALSE,
                      Quantity, shortage_cost = FALSE, inventory_cost = FALSE, 
                      ...
                      Backlogs=FALSE) 
{
  ...
  data$lost_order <- data$demand - data$sales
    ...
    data <- data.frame('period' = seq(1:(N + 1)), demand = demand, 
                       sales = sales, 'inventory_level' = I, inventory_position = IP, 
                       min = min, order = order, recieved = recieved,comu_backlogs=comu_backlogs)
    data$lost_order <- data$demand - data$sales
  ...
  if(plot== TRUE){
    ...
    print(plotly::ggplotly(data[is.na(data[,c('sales','demand','order')])==FALSE,] %>% ggplot(aes(x= period,y= demand,color="demand"))+geom_line()+
      ...
      theme_minimal()+geom_line(aes(y=order,color="order"))+ggtitle("Min Q policy")))
    )
  }
