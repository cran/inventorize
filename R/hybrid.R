...
#' Hybrid
#'
#' Simulating a Min Max periodic policy, 
...
hybrid_policy<-function (demand, mean = FALSE, sd=FALSE, leadtime, service_level, initial_inventory_level = FALSE, min= FALSE,Max=FALSE, Min_to_max=0.6,
                         Review_period,
                         ...
                         Backlogs=FALSE) 
{
  ...
  metrics <- data.frame(shortage_cost = sum(data$lost_order, 
                        ...
                        total_lost_sales = sum(data$lost_order, na.rm = TRUE), average_ordering_quantity= mean(order[order>0],na.rm = TRUE),ordering_interval= paste0(round(N/length(which(data$order > 0)),2),'_periods'),
  ...
  if(plot== TRUE){
    suppressWarnings(
      print(plotly::ggplotly(data[is.na(data[,c('sales','demand','order')])==FALSE,] %>% ggplot(aes(x= period,y= demand,color="demand"))+geom_line()+
              ...
              theme_minimal()+geom_line(aes(y=order,color="order"))+ggtitle("Hybrid Policy")))
    )
  }
...
hybrid_policy(demand = rpois(90,8),leadtime = 6,Review_period = 10,service_level = 0.9,
              distribution = 'nbinom',recalculate=TRUE)
