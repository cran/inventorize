...
#' @param Quantity  integer,Fixed ordering quantity.
...
sim_min_Q_dynamic<- function (demand,forecast, leadtime, service_level,initial_inventory_level=FALSE,Quantity,
                              one_step_forecast=TRUE,shortage_cost = FALSE, 
                              inventory_cost = FALSE, ordering_cost = FALSE,distribution= 'normal', 
                              error_metric= 'mse',smoothing_error=0.2,metric_windows= FALSE,plot= FALSE,Backlogs=FALSE) 
{
  ...
  data <- data.frame('period' = seq(1:(N + 1)), demand = demand, forecast=forecast,rolling_error= metric,
                     ...
                     min = min, order = order, recieved = recieved)
  data$lost_order <- data$demand - data$sales
   } else {
     
     data <- data.frame('period' = seq(1:(N + 1)), demand = demand, forecast=forecast,rolling_error= metric,
                        sales = sales, 'inventory_level' = I, inventory_position = IP,expected_demand_leadtime= dl,sigmadl= sigmadl,saftey_stock=saftey_stock,
                        min = min, order = order, recieved = recieved,comu_backlogs
                        )
     data$lost_order <- data$demand - data$sales 
  ...
  metrics <- data.frame(shortage_cost = sum(data$lost_order, 
                        ...
                        mae= mean(abs(demand-forecast),na.rm = TRUE),me= mean(demand-forecast),
                        mape= mean((abs(demand-forecast)/abs(demand))*100,na.rm=TRUE))
  ...
  if(plot== TRUE){
    suppressWarnings(
      print(plotly::ggplotly(data[is.na(data[,c('sales','demand','order')])==FALSE,] %>% ggplot(aes(x= period,y= demand,color="demand"))+geom_line()+
              ...
              theme_minimal()+geom_line(aes(y=order,color="order"))+ggtitle("Min Q policy dynamic")))
    )
  }
...
sim_min_Q_dynamic(demand = rpois(90,8),forecast = rpois(90,8),leadtime = 5,
service_level = 0.95,Quantity = 80,initial_inventory_level = 80,
 one_step_forecast=TRUE,shortage_cost = FALSE, smoothing_error = 0.3,
                inventory_cost = FALSE, ordering_cost = FALSE,distribution= 'nbinom', 
                 error_metric= 'rmse',metric_windows= FALSE)
